...
if(!$_POST){
    ...
    $num_eval = htmlentities($_POST ['cantidad_notas']);
    ...
    if (isset($_POST['insertar'])){
        ...
        if (isset($result)) {
            header('location:ingresonotas.php?grado='.$id_grado.'&materia='.$id_materia.'&curso='.$id_seccion.'&revisar=1&info=1');
        } else {
            header('location:ingresonotas.php?grado='.$id_grado.'&materia='.$id_materia.'&curso='.$id_seccion.'&revisar=1&err=1');
