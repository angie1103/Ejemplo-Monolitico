class DataTable{
        ...
        trs.forEach(tr =>{
            ...
            cells.forEach(cell =>{
                if(cell.children.length > 0){
                    ...
                    const statusEelement = [...cell.children][0];
                    const status = statusEelement.getAttribute('class');
                    if(status != null){
    ...
    removeSelect(id){
        const res = this.items.selected.filter(item => item.id !== id);
        this.selected = [... res];
    }
    renderPagesButtons(){
        ...
        const buttonsToShow = this.pagination.noButtonsBeforeDtos;
